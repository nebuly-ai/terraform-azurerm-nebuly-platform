...
By default, AKS workloads connect to PostgreSQL using username/password credentials stored in Key Vault.

You can optionally enable **hybrid authentication** so human operators can connect with Microsoft Entra ID (typically via PIM-eligible reader/writer groups), while AKS continues using password auth.

Requirements when `postgres_entra_access` is enabled:

* Pre-create Entra security groups for database readers and writers (object IDs are passed to the module).
* Assign an Entra administrator on the PostgreSQL server (`entra_admin` input).
* Apply SQL grants manually after provisioning using the `postgres_entra_grants_sql` output.
...
| <a name="output_postgres_entra_access"></a> [postgres\_entra\_access](#output\_postgres\_entra\_access) | Resolved Microsoft Entra access configuration for the PostgreSQL server. |
| <a name="output_postgres_entra_connection_notes"></a> [postgres\_entra\_connection\_notes](#output\_postgres\_entra\_connection\_notes) | Notes for connecting to PostgreSQL with Microsoft Entra authentication. |
| <a name="output_postgres_entra_grants_sql"></a> [postgres\_entra\_grants\_sql](#output\_postgres\_entra\_grants\_sql) | SQL statements used to bootstrap Entra principals and grants (for audit and troubleshooting). |
...
|------|-------------|------|---------|:--------:|
| <a name="input_aks_automatic_channel_upgrade"></a> [aks\_automatic\_channel\_upgrade](#input\_aks\_automatic\_channel\_upgrade) | (Optional) Defines the automatic upgrade channel for the AKS cluster.<br/><br/>Allowed values:<br/>  - "patch": upgrades to the latest patch version within the specified minor version in `kubernetes_version`. <br/>             (Requires `kubernetes_version` to be set only up to the minor version, e.g. "1.29".)<br/>  - "stable", "rapid", "node-image": upgrade automatically without `kubernetes_version`.<br/>             (When using these, both `kubernetes_version` and `orchestrator_version` must be `null`.)<br/><br/>Default: disabled (null). | `string` | `null` | no |
...
| <a name="input_aks_kubernetes_version"></a> [aks\_kubernetes\_version](#input\_aks\_kubernetes\_version) | The Kubernetes version to use. | <pre>object({<br/>    workers       = string<br/>    control_plane = string<br/>  })</pre> | <pre>{<br/>  "control_plane": "1.34.7",<br/>  "workers": "1.34.7"<br/>}</pre> | no |
...
| <a name="input_aks_maintenance_window_node_os"></a> [aks\_maintenance\_window\_node\_os](#input\_aks\_maintenance\_window\_node\_os) | The node OS versions that are allowed for automatic maintenance operations. <br/>  This variable is used to set the maintenance\_window\_node\_os property of the AKS cluster, which defines the node OS versions that are allowed for automatic maintenance operations. <br/>  By default, it is set to null, which means that all node OS versions are allowed for maintenance. <br/>  It can be set to a comma-separated list of allowed node OS versions (e.g., "Ubuntu,Windows") or to "None" to disable automatic maintenance operations on nodes. | <pre>object({<br/>    day_of_month = optional(number)<br/>    day_of_week  = optional(string)<br/>    duration     = number<br/>    frequency    = string<br/>    interval     = number<br/>    start_date   = optional(string)<br/>    start_time   = optional(string)<br/>    utc_offset   = optional(string, "+00:00")<br/>    week_index   = optional(string)<br/>    not_allowed = optional(set(object({<br/>      end   = string<br/>      start = string<br/>    })))<br/>  })</pre> | <pre>{<br/>  "day_of_week": "Sunday",<br/>  "duration": 4,<br/>  "frequency": "Weekly",<br/>  "interval": 1,<br/>  "start_time": "00:00",<br/>  "utc_offset": "+00:00"<br/>}</pre> | no |
...
| <a name="input_aks_net_profile_pod_cidr"></a> [aks\_net\_profile\_pod\_cidr](#input\_aks\_net\_profile\_pod\_cidr) | The CIDR to use for pod IP addresses. This field can only be set when network\_plugin is set to kubenet or network\_plugin is set to azure and network\_plugin\_mode is set to overlay. | `string` | `null` | no |
| <a name="input_aks_net_profile_profile_outbound_type"></a> [aks\_net\_profile\_profile\_outbound\_type](#input\_aks\_net\_profile\_profile\_outbound\_type) | The outbound (egress) routing method which should be used for this Kubernetes Cluster. Possible values are loadBalancer and userDefinedRouting. | `string` | `"loadBalancer"` | no |
...
| <a name="input_aks_oidc_issuer_enabled"></a> [aks\_oidc\_issuer\_enabled](#input\_aks\_oidc\_issuer\_enabled) | Enable the OIDC issuer URL and Workload Identity on the AKS cluster. | `bool` | `true` | no |
...
| <a name="input_postgres_entra_access"></a> [postgres\_entra\_access](#input\_postgres\_entra\_access) | Optional Microsoft Entra ID access for human operators on the PostgreSQL Flexible Server.<br/>  When enabled, the server uses hybrid authentication (password for AKS workloads + Entra for humans).<br/>  Reader and writer Entra groups (typically PIM-eligible) can be granted database privileges<br/>  with SQL statements exposed via `postgres_entra_grants_sql` output. | <pre>object({<br/>    enabled = optional(bool, false)<br/>    entra_admin = object({<br/>      object_id      = string<br/>      principal_name = string<br/>      principal_type = optional(string, "User")<br/>    })<br/>    reader_group_object_ids = optional(set(string), [])<br/>    writer_group_object_ids = optional(set(string), [])<br/>    databases               = optional(set(string), null)<br/>  })</pre> | `null` | no |
...
| <a name="input_private_dns_zones"></a> [private\_dns\_zones](#input\_private\_dns\_zones) | Private DNS zones to use for Private Endpoint connections. If not provided, a new DNS Zone <br/>  is created and linked to the respective subnet. | <pre>object({<br/>    flexible_postgres = optional(object({<br/>      name : optional(string)<br/>      id : optional(string)<br/>      resource_group_name : optional(string)<br/>      link_vnet : optional(bool, true)<br/>      create : optional(bool, true)<br/>    }), null)<br/>    openai = optional(object({<br/>      name : optional(string)<br/>      resource_group_name : optional(string)<br/>      link_vnet : optional(bool, true)<br/>      link_dns_zone_group : optional(bool, true)<br/>      create : optional(bool, true)<br/>    }), null)<br/>    key_vault = optional(object({<br/>      name : optional(string)<br/>      resource_group_name : optional(string)<br/>      link_vnet : optional(bool, true)<br/>      link_dns_zone_group : optional(bool, true)<br/>      create : optional(bool, true)<br/>    }), null)<br/>    blob = optional(object({<br/>      name : optional(string)<br/>      resource_group_name : optional(string)<br/>      link_vnet : optional(bool, false)<br/>      link_dns_zone_group : optional(bool, true)<br/>      create : optional(bool, true)<br/>    }), null)<br/>    dfs = optional(object({<br/>      name : optional(string)<br/>      resource_group_name : optional(string)<br/>      link_vnet : optional(bool, true)<br/>      link_dns_zone_group : optional(bool, true)<br/>      create : optional(bool, true)<br/>    }), null)<br/>  })</pre> | <pre>{<br/>  "blob": {},<br/>  "dfs": {},<br/>  "flexible_postgres": {},<br/>  "key_vault": {},<br/>  "openai": {}<br/>}</pre> | no |
...
- resource.azuread_application.main (/terraform-docs/main.tf#623)
- resource.azuread_group.aks_admins (/terraform-docs/main.tf#1262)
- resource.azuread_group_member.aks_admin_users (/terraform-docs/main.tf#1272)
- resource.azuread_service_principal.main (/terraform-docs/main.tf#635)
- resource.azuread_service_principal_password.main (/terraform-docs/main.tf#642)
- resource.azurerm_cognitive_account.main (/terraform-docs/main.tf#909)
- resource.azurerm_cognitive_deployment.gpt_4o (/terraform-docs/main.tf#933)
- resource.azurerm_cognitive_deployment.gpt_4o_mini (/terraform-docs/main.tf#950)
- resource.azurerm_key_vault.main (/terraform-docs/main.tf#553)
- resource.azurerm_key_vault_secret.azure_openai_api_key (/terraform-docs/main.tf#967)
- resource.azurerm_key_vault_secret.azuread_application_client_id (/terraform-docs/main.tf#655)
- resource.azurerm_key_vault_secret.azuread_application_client_secret (/terraform-docs/main.tf#670)
- resource.azurerm_key_vault_secret.backups_storage_primary_key (/terraform-docs/main.tf#1242)
- resource.azurerm_key_vault_secret.google_sso_client_id (/terraform-docs/main.tf#1475)
- resource.azurerm_key_vault_secret.google_sso_client_secret (/terraform-docs/main.tf#1486)
- resource.azurerm_key_vault_secret.jwt_signing_key (/terraform-docs/main.tf#1439)
- resource.azurerm_key_vault_secret.nebuly_azure_client_id (/terraform-docs/main.tf#688)
- resource.azurerm_key_vault_secret.nebuly_azure_client_secret (/terraform-docs/main.tf#699)
- resource.azurerm_key_vault_secret.okta_sso_client_id (/terraform-docs/main.tf#1453)
- resource.azurerm_key_vault_secret.okta_sso_client_secret (/terraform-docs/main.tf#1464)
- resource.azurerm_key_vault_secret.postgres_password (/terraform-docs/main.tf#879)
- resource.azurerm_key_vault_secret.postgres_user (/terraform-docs/main.tf#868)
- resource.azurerm_kubernetes_cluster_node_pool.linux_pools (/terraform-docs/main.tf#1390)
- resource.azurerm_management_lock.postgres_server (/terraform-docs/main.tf#811)
- resource.azurerm_monitor_metric_alert.postgres_server_alerts (/terraform-docs/main.tf#819)
- resource.azurerm_postgresql_flexible_server.main (/terraform-docs/main.tf#720)
- resource.azurerm_postgresql_flexible_server_active_directory_administrator.entra_admin (/terraform-docs/main.tf#890)
- resource.azurerm_postgresql_flexible_server_configuration.mandatory_configurations (/terraform-docs/main.tf#784)
- resource.azurerm_postgresql_flexible_server_configuration.optional_configurations (/terraform-docs/main.tf#777)
- resource.azurerm_postgresql_flexible_server_database.analytics (/terraform-docs/main.tf#805)
- resource.azurerm_postgresql_flexible_server_database.auth (/terraform-docs/main.tf#799)
- resource.azurerm_postgresql_flexible_server_database.extras (/terraform-docs/main.tf#791)
- resource.azurerm_private_dns_zone.blob (/terraform-docs/main.tf#464)
- resource.azurerm_private_dns_zone.dfs (/terraform-docs/main.tf#483)
- resource.azurerm_private_dns_zone.flexible_postgres (/terraform-docs/main.tf#426)
- resource.azurerm_private_dns_zone.key_vault (/terraform-docs/main.tf#445)
- resource.azurerm_private_dns_zone.openai (/terraform-docs/main.tf#502)
- resource.azurerm_private_dns_zone.web_app_routing (/terraform-docs/main.tf#521)
- resource.azurerm_private_dns_zone_virtual_network_link.blob (/terraform-docs/main.tf#470)
- resource.azurerm_private_dns_zone_virtual_network_link.dfs (/terraform-docs/main.tf#489)
- resource.azurerm_private_dns_zone_virtual_network_link.flexible_postgres (/terraform-docs/main.tf#432)
- resource.azurerm_private_dns_zone_virtual_network_link.key_vault (/terraform-docs/main.tf#451)
- resource.azurerm_private_dns_zone_virtual_network_link.openai (/terraform-docs/main.tf#508)
- resource.azurerm_private_dns_zone_virtual_network_link.web_app_routing (/terraform-docs/main.tf#532)
- resource.azurerm_private_endpoint.backups_blob (/terraform-docs/main.tf#1155)
- resource.azurerm_private_endpoint.backups_dfs (/terraform-docs/main.tf#1181)
- resource.azurerm_private_endpoint.key_vault (/terraform-docs/main.tf#579)
- resource.azurerm_private_endpoint.models_blob (/terraform-docs/main.tf#1055)
- resource.azurerm_private_endpoint.models_dfs (/terraform-docs/main.tf#1081)
- resource.azurerm_private_endpoint.openai (/terraform-docs/main.tf#978)
- resource.azurerm_role_assignment.aks_network_contributor (/terraform-docs/main.tf#1385)
- resource.azurerm_role_assignment.key_vault_secret_officer__current (/terraform-docs/main.tf#613)
- resource.azurerm_role_assignment.key_vault_secret_user__aks (/terraform-docs/main.tf#605)
- resource.azurerm_role_assignment.nebuly_secrets_officer (/terraform-docs/main.tf#648)
- resource.azurerm_role_assignment.storage_container_models__data_contributor (/terraform-docs/main.tf#1048)
- resource.azurerm_role_assignment.web_app_routing_identity__dns_zone (/terraform-docs/main.tf#544)
- resource.azurerm_storage_account.backups (/terraform-docs/main.tf#1119)
- resource.azurerm_storage_account.main (/terraform-docs/main.tf#1019)
- resource.azurerm_storage_container.clickhouse (/terraform-docs/main.tf#1149)
- resource.azurerm_storage_container.models (/terraform-docs/main.tf#1042)
- resource.azurerm_storage_management_policy.backups (/terraform-docs/main.tf#1207)
- resource.azurerm_subnet.aks_nodes (/terraform-docs/main.tf#350)
- resource.azurerm_subnet.flexible_postgres (/terraform-docs/main.tf#374)
- resource.azurerm_subnet.private_endpoints (/terraform-docs/main.tf#366)
- resource.azurerm_virtual_network.main (/terraform-docs/main.tf#338)
- resource.random_password.postgres_server_admin_password (/terraform-docs/main.tf#715)
- resource.time_sleep.wait_aks_creation (/terraform-docs/main.tf#1361)
- resource.tls_private_key.aks (/terraform-docs/main.tf#1258)
- resource.tls_private_key.jwt_signing_key (/terraform-docs/main.tf#1435)
- data source.azuread_group.postgres_entra_readers (/terraform-docs/main.tf#253)
- data source.azuread_group.postgres_entra_writers (/terraform-docs/main.tf#258)
- data source.azuread_user.aks_admins (/terraform-docs/main.tf#248)
- data source.azurerm_client_config.current (/terraform-docs/main.tf#240)
- data source.azurerm_private_dns_zone.blob (/terraform-docs/main.tf#323)
- data source.azurerm_private_dns_zone.dfs (/terraform-docs/main.tf#329)
- data source.azurerm_private_dns_zone.flexible_postgres (/terraform-docs/main.tf#305)
- data source.azurerm_private_dns_zone.key_vault (/terraform-docs/main.tf#317)
- data source.azurerm_private_dns_zone.openai (/terraform-docs/main.tf#311)
- data source.azurerm_resource_group.main (/terraform-docs/main.tf#237)
- data source.azurerm_subnet.aks_nodes (/terraform-docs/main.tf#263)
- data source.azurerm_subnet.flexible_postgres (/terraform-docs/main.tf#291)
- data source.azurerm_subnet.private_endpoints (/terraform-docs/main.tf#277)
- data source.azurerm_virtual_network.main (/terraform-docs/main.tf#242)
